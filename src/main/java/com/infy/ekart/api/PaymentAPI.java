...
public class PaymentAPI {
	...
	public ResponseEntity<String> addNewCard(@RequestBody CardDTO cardDTO,
		...
		String message=environment.getProperty("PaymentAPI.CUSTOMER_CARD_DELETED_SUCCESS")+id;
	...
	public ResponseEntity<String> payForOrder(
		...
		logger.info("Recieved request for paying for order :"+orderId);  
//		String getOrderDetailsUrls="http://localhost:3333/Ekart/order-api/{orderId}";
...
//		OrderDTO orderDTO1 = orderResponse.getBody();
		...
		tDTO.setOrder(orderDTO);
		tDTO.setTransactionDate(orderDTO.getDateOfDelivery());
		...
		template.put("http://localhost:3333/Ekart/order-api/order/"+tDTO.getOrder().getOrderId()+"/update/order-status",
				tDTO.getTransactionStatus().toString());
		
		String message=environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFULL_ONE")+tDTO.getTotalPrice()+""
		+environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFULL_TWO")+tDTO.getOrder().getOrderId()+environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFULL_THREE")+id;
		return new ResponseEntity<>(message,HttpStatus.OK);
	}
